...
RETURNS TABLE(id INT, first_name VARCHAR, last_name VARCHAR, phone VARCHAR, email VARCHAR, address TEXT) AS $$
BEGIN
    RETURN QUERY
...
RETURNS TABLE(id INT, first_name VARCHAR, last_name VARCHAR, phone VARCHAR, email VARCHAR, address TEXT, total_count BIGINT) AS $$
DECLARE
    offset_val INT;
    total BIGINT;
BEGIN
    ...
    RETURN QUERY
    SELECT c.id, c.first_name, c.last_name, c.phone, c.email, c.address, total AS total_count
    FROM contacts c
    ORDER BY c.first_name
