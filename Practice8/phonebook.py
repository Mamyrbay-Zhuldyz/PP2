...
def create_functions_and_procedures():
    ...
    try:
        ...
        print(f"Error creating functions/procedures: {e}")
        if conn:
            conn.rollback()
    finally:
        if conn:
...
def add_contact():
    """Add a new contact using upsert procedure"""
    conn = None
    try:
        print("\n--- Add New Contact (using upsert) ---")
        ...
        cur.execute("CALL upsert_contact(%s, %s, %s, %s, %s)", 
                   (first_name, last_name, phone, email or None, address or None))
        
        conn.commit()
        print(f"✅ Contact '{first_name} {last_name}' added/updated successfully!")
        cur.close()
    except Exception as e:
        print(f"Error: {e}")
        if conn:
...
def delete_contact():
    """Delete contact using procedure"""
    conn = None
    try:
        print("\n--- Delete Contact (using procedure) ---")
        print("Delete by:")
        ...
        if choice == '1':
            ...
            cur.execute("CALL delete_contact_by(%s, NULL, NULL)", (first_name,))
        elif choice == '2':
            phone = input("Enter phone number: ").strip()
            cur.execute("CALL delete_contact_by(NULL, NULL, %s)", (phone,))
        elif choice == '3':
            first_name = input("Enter first name: ").strip()
            last_name = input("Enter last name: ").strip()
            cur.execute("CALL delete_contact_by(%s, %s, NULL)", (first_name, last_name))
        else:
            print("❌ Invalid option!")
            return
...
def search_contacts():
    """Search contacts using function"""
    conn = None
    try:
        print("\n--- Search Contacts (using function) ---")
        print("Search by:")
        print("1. Name or phone (pattern search)")
        print("2. Email")
...
def show_all_contacts(show_id=False):
    ...
    try:
        ...
        print("2. Paginated view (using function)")
        choice = input("Choose option (1-2): ").strip()
        ...
        if choice == '2':
            try:
                page_num = int(input("Page number (1,2,3...): ").strip())
                page_size = int(input("Records per page: ").strip())
            except ValueError:
                print("❌ Invalid number!")
                return
            
            cur.execute("SELECT * FROM get_contacts_paginated(%s, %s)", (page_num, page_size))
            rows = cur.fetchall()
            
            if rows:
                total = rows[0][6] if rows else 0
                print(f"\n📊 Page {page_num} | Showing {len(rows)} of {total} contacts:")
                print("-" * 70)
                print(f"{'ID':<5} {'Name':<25} {'Phone':<15} {'Email':<25}")
                print("-" * 70)
                for row in rows:
                ...
                print(f"\n❌ No contacts found on page {page_num}")
...
def bulk_insert_demo():
    """Bulk insert with validation using procedure"""
    conn = None
    try:
        print("\n--- Bulk Insert with Validation ---")
        print("Enter contacts (one by one). Leave first name empty to finish.")
        
        contacts = []
        while True:
            print("\n--- New contact ---")
            first_name = input("First name (empty to finish): ").strip()
            if not first_name:
                break
            ...
            contacts.append([first_name, last_name, phone, email or None, address or None])
...
def menu():
    ...
    while True:
        print("\n" + "="*50)
        print("        PHONEBOOK MANAGER (with Functions & Procedures)")
        print("="*50)
        print("1. Show all contacts (with pagination)")
        print("2. Add new contact (upsert)")
        print("3. Search contacts (using function)")
        print("4. Update contact")
        print("5. Delete contact (using procedure)")
        print("6. Import from CSV")
        print("7. Bulk insert with validation (using procedure)")
        print("8. Exit")
        print("="*50)
